    ...
    for target_url in target_urls:
        try:
            ...
            for article in articles:
                ...
                # --- Fetch Image ---
                image_element = article.find('img', class_='c-entry-box--compact__image') 
                image_url = image_element['src'] if image_element else None

                # --- Download and Save the Image ---
                if image_url:
                    image_filename = f"{title.replace(' ', '_').lower()}.jpg"
                    fetch_and_save_image(image_url, f"images/{image_filename}")
                ...
                insert_article(title, link, content, image_url, source, summary)

        except requests.exceptions.RequestException as e:
            print(f"Error scraping {target_url}: {e}")

# --- Generate HTML (Modified to use template) ---
